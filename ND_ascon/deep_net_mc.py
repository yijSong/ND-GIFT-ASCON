...
def make_resnet(group_size=2, num_blocks=5, num_filters=32, num_outputs=1, d1=64, d2=64, word_size=64, ks=3, depth=5, reg_param=0.001, final_activation='sigmoid'):
...
def train_ac_distinguisher(num_epochs, num_rounds=4, group_size=2, depth=1):
    #create the network
    net = make_resnet(group_size=group_size, depth=depth, reg_param=10**-5);
    net.compile(optimizer='adam', loss='mse', metrics=['acc']);
    #generate training and validation data
    X, Y = ac.make_td(n=10**7 // group_size, s=group_size, nr=num_rounds)
    X_eval, Y_eval = ac.make_td(n=10**6 // group_size, s=group_size, nr=num_rounds)
...
if __name__ == '__main__':
    train_ac_distinguisher(num_epochs=50, num_rounds=4, group_size=1)