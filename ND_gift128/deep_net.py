...
def make_resnet(num_blocks=16, num_filters=64, num_outputs=1, d1=64, d2=64, word_size=8, ks=5, depth=5, reg_param=0.0001, final_activation='sigmoid'):
...
def train_gift_distinguisher(num_epochs, num_rounds=7, depth=1):
    # create the network
    net = make_resnet(depth=depth, reg_param=10**-5)
    net.compile(optimizer='adam', loss='mse',metrics=['acc'])
    # generate training and validation data
    X, Y = gift.make_train_data(10**7, num_rounds)
    X_eval, Y_eval = gift.make_train_data(10**6, num_rounds)
    ...
    # save model
    # net.save('./saved_model/' + str(num_rounds) + '_distinguisher.h5')

    return(net, h)


if __name__ == '__main__':
    train_gift_distinguisher(num_epochs=50, num_rounds=7)
